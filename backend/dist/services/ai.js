...
function getSystemPrompt(action, tone, mode) {
...
export async function processAIRequest(action, text, tone, mode, customPrompt) {
...
export async function processAdvancedAIRequest(action, text, options) {
    const client = getOpenAIClient();
    const coinsUsed = ADVANCED_COIN_COSTS[action] || 20;
    try {
        switch (action) {
...
export function getActionCost(action) {
    return COIN_COSTS[action] || ADVANCED_COIN_COSTS[action] || 10;
}
