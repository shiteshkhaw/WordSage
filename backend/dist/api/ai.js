...
aiRouter.post('/process', requireAuth, async (req, res) => {
    try {
        if (!req.user) {
...
aiRouter.post('/advanced', requireAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
aiRouter.post('/team-process', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const result = await processTeamAwareAI(action, text, teamId, req.user.id, prisma, mode, tone, customPrompt);
...
aiRouter.get('/team-suggestions', requireAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        const suggestions = await getTeamContentSuggestions(query, teamId, prisma);
...
aiRouter.post('/check-violations', requireAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
