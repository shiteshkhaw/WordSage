...
analyticsRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
        if (!req.user) {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayUsage = aiUsageStats.filter((u: any) => {
            ...
            const wordsProcessed = dayUsage.reduce((sum: number, u: any) => {
        ...
        // Calculate insights
        const thisWeekRequests = transactions.filter((t: any) => t.transaction_type === 'usage').length;
