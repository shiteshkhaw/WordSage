...
profileRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        let referralCode = (userProfile.user_profiles as any)?.referral_code;
        if (!referralCode && userProfile.user_profiles) {
            referralCode = generateReferralCode();
            // Update user_profiles with the new referral code
            await prisma.user_profiles.update({
                where: { id: req.user!.id },
                data: { referral_code: referralCode } as any,
            });
        }
        ...
        res.json({
            data: {
                ...
                total_referrals: (userProfile.user_profiles as any)?.total_referrals || 0,
            },
        });
    } catch (error: any) {
...
// Update user profile
profileRouter.put('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
...
// Get user analytics
profileRouter.get('/analytics', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
